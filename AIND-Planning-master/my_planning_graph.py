...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        state_nodes = set()
        ### Loop through each action in action level
        for action_node in self.a_levels[level-1]:
            ### Effective node found and update parent & children values
        ...
        return self.s_levels.append(state_nodes)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ### Check if a1 +effect is in a2 -precondition
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                testFlag1=True
        ### Check if a2 +effect is in a1 -precondition
        for effect in node_a2.action.effect_add:
        ...
        for effect in node_a2.action.effect_rem:
            if effect in node_a1.action.precond_pos:
                testFlag4=True
        if testFlag1 or testFlag2 or testFlag3 or testFlag4:
            return True
        else:
            return False
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    ### Merge is_pos and symbol and transform it in the goal's format
